...
Product returns are a normal part of e-commerce, particularly because customers cannot physically inspect products before purchasing them. However, repeated or unusually high return activity can increase costs associated with reverse logistics, inventory handling, product depreciation and refund processing.

The main challenge is that high return activity does not automatically indicate return abuse. Customers may legitimately return several products, and the dataset used in this project does not contain a confirmed fraud or return-abuse label. Therefore, the project was framed as a **high-risk return behaviour prediction problem**, rather than a fraud-detection problem.

The analytical objective was to investigate whether patterns within historical customer transaction behaviour could be used to identify customers associated with higher return activity. This required moving beyond individual transactions and examining behaviour at the **customer level**, where purchasing frequency, spending patterns, discount behaviour, product preferences, shipping behaviour and other characteristics could be analysed together.

From a practical perspective, the resulting model is intended as a **risk-screening and decision-support approach**. A high-risk prediction could be used to identify customers or transactions that may require further review, rather than being treated as evidence that a customer has committed return abuse.
# Using Machine Learning Techniques to Predict High-Risk Return Behaviour in Online Retail E-Commerce: A Case Study of Electronic Devices
